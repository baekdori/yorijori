...
// 댓글 생성 라우터
router.post('/', (req, res) => {
    const { comment_idx, user_id, comment_text, food_idx, food_emotion } = req.body;

    if (!comment_idx || !user_id || !comment_text || !food_idx || !food_emotion) {
        return res.status(400).json({ error: '모든 필드 입력해야함' });
    }
